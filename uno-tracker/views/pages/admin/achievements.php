            ...
            <div class="relative overflow-hidden bg-white rounded-2xl border-2 border-<?= $rarity['color'] ?>-200 shadow-md hover:shadow-xl transition-all duration-300 hover:scale-[1.02] group">
                ...
                <div class="bg-<?= $rarity['color'] ?>-50 px-4 py-3 border-b-2 border-<?= $rarity['color'] ?>-200">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2"><span class="text-2xl"><?= $achievement['icon'] ?></span>
                            <div>
                                <div class="font-bold text-gray-800 text-sm"><?= htmlspecialchars($achievement['name']) ?></div>
                                <div class="text-xs font-medium text-gray-500"><?= $categoryLabels[$achievement['category']] ?? $achievement['category'] ?></div>
                            </div>
                        </div>
                ...
                <div class="p-4 space-y-3">
                    <p class="text-xs font-medium text-gray-600"> (ID: #<?= $achievement['id'] ?>) <?= htmlspecialchars($achievement['description'] ?? '-') ?></p>
                    <div class="grid grid-cols-2 gap-2 text-xs">
                        <div class="bg-gray-50/80 rounded-2xl p-2 border border-gray-200">
                            <div class="font-bold text-gray-500">شرط</div>
                            <div class="font-bold text-gray-800"><?= $conditionTypeLabels[$achievement['condition_type']] ?? $achievement['condition_type'] ?></div>
                        </div>
                        <div class="bg-gray-50/80 rounded-2xl p-2 border border-gray-200">
                            <div class="font-bold text-gray-500">مقدار</div>
                            <div class="font-bold text-gray-800"><?= $achievement['condition_value'] ?></div>
                        </div>
                        <div class="bg-gray-50/80 rounded-2xl p-2 border border-gray-200">
                            <div class="font-bold text-gray-500">XP پاداش</div>
                            <div class="font-bold text-indigo-600">+<?= $achievement['xp_reward'] ?></div>
                        </div>
                        <div class="bg-gray-50/80 rounded-2xl p-2 border border-gray-200">
                            <div class="font-bold text-gray-500">کسب شده</div>
                            <div class="font-bold text-green-600"><?= (int)$achievement['unlocked_count'] ?> نفر</div>
                        </div>
                    </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col border-2 border-gray-200/70" @click.away="closeModal()">
            <div class="px-6 py-4 border-b-2 border-gray-200 flex-shrink-0">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="closeModal()" class="text-gray-400 hover:text-gray-600 transition p-1"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form :action="isEditing ? '/admin/achievements/' + formData.id : '/admin/achievements/create'" method="POST" id="achievement-form" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div><label class="block text-sm font-bold text-gray-700 mb-2">دسته‌بندی</label><select name="category" x-model="formData.category" class="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 appearance-none cursor-pointer hover:border-indigo-300 bg-white">
                                ...
                                <option value="winning">🏆 پیروزی</option>
                                <option value="streak">🔥 زنجیره پیروزی</option>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div><label class="block text-sm font-bold text-gray-700 mb-2">نوع شرط <span class="text-red-500">*</span></label><select name="condition_type" x-model="formData.condition_type" required class="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 appearance-none cursor-pointer hover:border-indigo-300 bg-white">
                                <option value="total_games">تعداد بازی</option>
                                <option value="total_wins">تعداد برد</option>
                                <option value="best_streak">بهترین استریک</option>
                                <option value="current_streak">استریک فعلی</option>
                                <option value="team_games">بازی تیمی</option>
                                <option value="team_wins">برد تیمی</option>
                                <option value="total_points">امتیاز کل</option>
                            </select></div>
        ...
        return {
            ...
            openEditModal(a) {
                ...
                this.formData = {
                    ...
                    xp_reward: parseInt(a.xp_reward) || 10,
                    condition_type: a.condition_type || 'total_games',
                    condition_value: parseInt(a.condition_value) || 1,
                    is_active: parseInt(a.is_active) || 0
                };
                this.showModal = true;
            },
            closeModal() {
            ...
            deleteAchievement(id, name) {
                Swal.fire({
                        title: 'حذف نشان',
                        html: `آیا از حذف نشان <strong class="text-red-600">${name}</strong> مطمئن هستید؟<br><br><span class="text-xs text-gray-500">این عملیات غیرقابل بازگشت است و تمام پیشرفت‌های کاربران نیز حذف می‌شود.</span>`,
                        icon: 'warning',
                    ...
                    .then((result) => {
                        if (result.isConfirmed) {
                            ...
                            form.method = 'POST';
                            form.action = `/admin/achievements/${id}/delete`;
