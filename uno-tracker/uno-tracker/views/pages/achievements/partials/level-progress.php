...
$totalXp = $xpInfo['total_xp'];
$xpToNext = $xpInfo['xp_to_next_level'];
?>
...
<div class="relative overflow-hidden">
    ...
    <div class="relative z-10">
        <h2 class="text-lg sm:text-xl font-black text-gray-800 mb-4 flex items-center gap-2.5 tracking-tight">
            <span class="text-2xl sm:text-3xl"><?= $levelData->icon ?? '⭐' ?></span>
            سطح و پیشرفت
        </h2>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-5 mb-6">
            <div class="w-20 h-20 sm:w-24 sm:h-24 rounded-full flex items-center justify-center shadow-2xl flex-shrink-0"
                 style="background: linear-gradient(135deg, <?= $levelData->color ?? '#6366f1' ?>, <?= $levelData->color ?? '#6366f1' ?>dd);">
                <div class="text-center text-white">
                    <div class="text-3xl sm:text-4xl font-black drop-shadow"><?= $xpInfo['current_level'] ?></div>
                    <div class="text-[10px] font-bold opacity-80">سطح</div>
                </div>
            </div>
            
            <div class="flex-1 text-center sm:text-right">
                <h3 class="text-xl sm:text-2xl font-black" style="color: <?= $levelData->color ?? '#6366f1' ?>">
                    <?= htmlspecialchars($levelData->title ?? 'سطح ' . $xpInfo['current_level']) ?>
                </h3>
                <p class="text-gray-500 text-sm font-medium mt-1">
                    <?= number_format($totalXp) ?> امتیاز تجربه از <?= number_format($levelData->max_xp + 1) ?>
                </p>
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="w-full bg-gray-200 rounded-full h-3 sm:h-4 overflow-hidden shadow-inner">
                <div class="h-full rounded-full transition-all duration-700 flex items-center justify-center text-[10px] text-white font-black"
                     style="width: <?= max(5, $progressPercentage) ?>%; background: linear-gradient(90deg, <?= $levelData->color ?? '#6366f1' ?>, <?= $levelData->color ?? '#6366f1' ?>cc);">
            ...
            <div class="text-xs text-gray-500 font-medium mt-1.5 text-center">
                <strong class="text-indigo-600"><?= number_format($xpToNext) ?></strong> امتیاز دیگر برای رسیدن به سطح <?= $xpInfo['current_level'] + 1 ?>
            </div>
        </div>
            ...
            <div class="relative">
                ...
                <div id="levels-container" 
                    ...
                    <?php foreach ($allLevels as $level): ?>
                        <?php 
                        $isCurrentLevel = $level->level === $xpInfo['current_level'];
                        $isUnlocked = $level->level < $xpInfo['current_level'];
                        ?>
